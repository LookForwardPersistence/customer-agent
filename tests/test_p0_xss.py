...
def test_dynamic_fields_use_text_content():
    ...
    dynamic_setters = [
        ...
        "err.message" in SOURCE and "textContent" in SOURCE,
    ]
    for stmt in dynamic_setters[:-1]:
        assert stmt in SOURCE, f"missing safe setter: {stmt}"


def test_no_innerhtml_with_user_content():
    # innerHTML 只允许出现在两处静态内容：typing 指示器与客户欢迎语（静态 map），
    # 不得用于任何来自用户/模型/后端的动态数据。
    matches = list(re.finditer(r"\.innerHTML\s*=", SOURCE))
...
def test_trace_masking_by_default():
    assert "state.rawTrace" in SOURCE
    assert "maskSensitive(t.detail)" in SOURCE
