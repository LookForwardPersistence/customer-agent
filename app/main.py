...
class ChatRequest(BaseModel):
    # Schema-level guard: empty body or >2000 chars never reaches the agent.
    # (Whitespace-only still passes here; the endpoint 400s after stripping.)
    message: str = Field(min_length=1, max_length=MAX_MESSAGE_CHARS)


class ActionRequest(BaseModel):
    # action_id is generated by secrets.token_urlsafe(9) (~12 chars).
    action_id: str = Field(min_length=8, max_length=64, pattern=r"^[A-Za-z0-9_-]+$")


class NewSessionRequest(BaseModel):
    # Mock directory uses CUST-001..CUST-004; unknown ids fail at issuance.
    customer_id: str = Field(default="CUST-001", pattern=r"^CUST-\d{3,}$")
...
def _run_agent(sid: str, human_text: str) -> dict[str, Any]:
    ...
    try:
        ...
        # LLM channel failure (timeout / connection / provider 5xx). The
        # request must not hang or 500 — degrade to a deterministic reply.
        code = type(e).__name__[:60]
        sessions.log(sid, AgentDegraded(code).to_dict())
        return {
            "reply": (
                "抱歉，小极刚刚与服务的连接中断了，这条消息没能处理成功。"
                "您的消息已记录，请稍后重发一句，或直接说\"转人工\"由人工客服跟进。"
            ),
            "sources": [],
            "trace": [],
            "pending_action": None,
...
def chat(req: ChatRequest, customer: AuthenticatedCustomer = Depends(get_customer)):
    ...
    # Length is enforced by ChatRequest (422); whitespace-only still 400s here.
